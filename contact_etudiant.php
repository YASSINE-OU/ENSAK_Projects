...
<!-- contact -->
  ...
  if (isset($_POST['submit'])) 
  {
    $dbh=new PDO("mysql:host=localhost;dbname=pfe_db","root","");
      ...
      $sth =$dbh->query("SELECT nom_etud FROM etudiant WHERE login_etud='$Login' ");
      
      $resultat=$sth->fetch(PDO::FETCH_ASSOC);
      foreach($resultat as $row)
        $nom=$row;
      $sth =$dbh->query("SELECT prenom_etud FROM etudiant WHERE login_etud='$Login' ");
